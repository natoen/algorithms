...
Tree.prototype.getClosestCommonAncestor = function(node1, node2) {
  ...
  const ancestors1 = this.getAncestorPath(node1);
  const ancestors2 = this.getAncestorPath(node2);

  if (!ancestors1 || !ancestors2) {
    return null;
  }

  for (let i = ancestors1.length - 1; i > -1; i--) {
...
Tree.prototype.getAncestorPath = function(descendant, notfirst) {
  const result = [];

  if (this.isDescendant(descendant)) {
    result.push(this);
  } else if (!notfirst) {
    return null;
  }
  ...
  for (let i = 0; i < this.children.length; i++) {
    result = result.concat(this.children[i].getAncestorPath(descendant, true));
  }
