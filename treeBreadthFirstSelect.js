...
Tree.prototype.BFSelect = function(filter) {
  ...
  while (queued) {
    filter(queued[0].value, queued[1]) ? result.push(queued[0].value) : null;

    const children = queued[0].children.length ? queued[0].children : [];
    for (const child of children) {
      queue.enqueue([child, queued[1] + 1]);
    }
...
// BFSelect accepts a filter function, calls that
// function on each of the nodes in Breadth-First order,
// and returns a flat array of node values of the tree for
// which the filter returns true.
...
//   return depth === 1;
// }) //=> [2, 3]
// For more information on breadth-first traversal,
// visit this link: Breadth-First Traversal of a Tree.
